...
const iconMap = {
    ...
    question: '<i class="fas fa-question-circle text-yellow-500" style="font-size: 48px;"></i>',
};
...
const createAlert = (type = 'info', title = 'Alert', message = '', timer = 3000, showConfirm = true) => {
    ...
    const alertHTML = `
        <div id="${alertId}" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" style="display: flex;">
            <div class="relative w-[560px] rounded-[20px] bg-gradient-to-r from-slate-700 to-neutral-400 p-6 font-sans shadow-lg" style="border-radius: 20px; background-clip: padding-box, border-box;">
                <div class="mx-auto h-[83px] w-[83px] flex items-center justify-center">
                    ${icon}
                </div>

                <h2 class="mt-4 text-center font-sans text-2xl font-bold text-white">
                    ${title}
                </h2>

                <p class="mt-2 text-center text-base font-medium text-white">
                    ${message}
                </p>

                <div class="mt-6 flex justify-center">
                    <button class="alert-close-btn rounded-[5px] bg-green-500 px-10 py-2 font-medium text-white cursor-pointer">OK</button>
                </div>
            </div>
        </div>
    `;
    ...
    const alertHTML = `
        <div id="${alertId}" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div class="relative w-[560px] rounded-[20px] bg-gradient-to-r from-slate-700 to-neutral-400 p-6 font-sans shadow-lg" style="border-radius: 20px; background-clip: padding-box, border-box;">
                ...
                <div class="w-1/2 mt-6 flex justify-center gap-4 justify-self-center">
                    <button class="w-1/2 alert-cancel-btn rounded-lg bg-red-500 px-6 py-2 text-white cursor-pointer font-medium">Tidak</button>
                    <button class="w-1/2 alert-confirm-btn rounded-[5px] bg-green-500 px-6 py-2 font-medium text-white cursor-pointer">Ya</button>
                </div>
            </div>
        </div>
    `;
...
const showValidationErrors = (errors, title = 'Validasi Gagal') => {
    ...
    const alertHTML = `
        <div id="${alertId}" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div class="relative w-[560px] rounded-[20px] bg-gradient-to-r from-slate-700 to-neutral-400 p-6 font-sans shadow-lg max-h-96 overflow-y-auto" style="border-radius: 20px; background-clip: padding-box, border-box;">
                <div class="mx-auto h-[83px] w-[83px] flex items-center justify-center">
                    ${icon}
                </div>
                ...
                <div class="mt-4 text-center text-base font-medium text-white" style="text-align: left; word-break: break-word;">
                    ${errorMessage.replace(/\n/g, '<br>')}
                </div>

                <div class="mt-6 flex justify-center">
                    <button class="alert-close-btn rounded-[5px] bg-red-500 px-10 py-2 font-medium text-white cursor-pointer">OK</button>
                </div>
            </div>
        </div>
    `;
