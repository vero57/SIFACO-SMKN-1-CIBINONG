...
<div class="content-section mx-4">
    <div class="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 px-10 border border-slate-700">
        <h3 class="text-3xl text-center font-semibold text-white mb-4">{{ isset($detail) ? 'Edit Detail Siswa' : 'Tambah Detail Siswa' }}</h3>
        <form action="{{ isset($detail) ? route('dashboard.siswa.detail.update', $user->id) : route('dashboard.siswa.detail.store', $user->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-2 gap-6 pb-4">
                <div class="">
                    ...
                    <input type="text" value="{{ $user->name }}" disabled class="w-full bg-slate-900 text-slate-200 border border-slate-700 rounded-lg px-3 py-3 text-sm" />
                    <input type="hidden" name="user_id" value="{{ $user->id }}">
                </div>
                <div class="">
                    <label class="block text-slate-300 mb-1 text-sm">NIS</label>
                    <input type="text" name="nis" value="{{ old('nis', $detail->nis ?? '') }}" required class="w-full bg-slate-900 text-slate-200 border border-slate-700 rounded-lg px-3 py-3 text-sm" />
                </div>
                <div class="">
                    <label class="block text-slate-300 mb-1 text-sm">NISN</label>
                    <input type="text" name="nisn" value="{{ old('nisn', $detail->nisn ?? '') }}" class="w-full bg-slate-900 text-slate-200 border border-slate-700 rounded-lg px-3 py-3 text-sm" />
                </div>
                <div class="">
                    <label class="block text-slate-300 mb-1 text-sm">Gender</label>
                    <select name="gender" required class="w-full bg-slate-900 text-slate-200 border border-slate-700 rounded-lg px-3 py-3 text-sm">
                ...
                <div class="">
                    ...
                    <input type="text" name="birth_place" value="{{ old('birth_place', $detail->birth_place ?? '') }}" required class="w-full bg-slate-900 text-slate-200 border border-slate-700 rounded-lg px-3 py-3 text-sm" />
                </div>
                <div class="">
                    <label class="block text-slate-300 mb-1 text-sm">Tanggal Lahir</label>
                    <input type="date" name="birth_date" value="{{ old('birth_date', $detail->birth_date ?? '') }}" required class="w-full bg-slate-900 text-slate-200 border border-slate-700 rounded-lg px-3 py-3 text-sm" />
                </div>
                <div class="">
                    <label class="block text-slate-300 mb-1 text-sm">Alamat</label>
                    <input type="text" name="address" value="{{ old('address', $detail->address ?? '') }}" required class="w-full bg-slate-900 text-slate-200 border border-slate-700 rounded-lg px-3 py-3 text-sm" />
                </div>
                <div class="">
                    <label class="block text-slate-300 mb-1 text-sm">Foto</label>
                    <input type="file" name="photo" accept="image/*" class="w-full bg-slate-900 text-slate-200 border border-slate-700 rounded-lg px-3 py-3 text-sm" />
