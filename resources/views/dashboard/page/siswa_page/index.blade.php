...
<div class="content-section">
    <div class="flex flex-row items-center justify-between backdrop-blur-sm rounded-xl px-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h3 class="text-2xl font-semibold text-black">Manajemen Data Siswa</h3>
                <p class="text-gray-700">Daftar Siswa.</p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <form action="{{ route('dashboard.siswa.import') }}" method="POST" enctype="multipart/form-data" class="">
                @csrf
                <label class="text-blue-500 border border-blue-500 hover:text-blue-600 hover:border-blue-600 px-3 py-2 rounded text-sm cursor-pointer mb-0">
            ...
            <a href="{{ route('dashboard.users.create') }}">
                <button type="button" class="bg-blue-600 hover:bg-blue-500 text-white px-3 py-2 rounded text-sm"><i class="fa fa-plus"></i> Tambah Data Siswa</button>
            </a>
        </div>
    </div>
    ...
    <div class="mt-6 backdrop-blur-sm rounded-xl p-4 border border-gray-300">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-4">
            ...
            <form class="flex items-center gap-3" id="siswa-search-form">
                <input type="search" id="siswa-search-input" name="search" placeholder="Cari nama atau NIS" value="{{ request('search', $search ?? '') }}" class="bg-white placeholder:text-slate-600 text-slate-700 border border-gray-300 rounded px-3 py-2 text-sm" />
        ...
        <div class="overflow-x-auto -mx-4">
            <table class="min-w-full table-auto border-collapse border border-gray-300">
                <thead class="bg-[#ecedf7] border border-gray-300">
                    <tr class="text-left text-slate-700 text-sm uppercase tracking-wider">
                ...
                <tbody class="divide-y divide-gray-400 bg-white" id="siswa-table-body">
                    ...
                    @foreach($users as $user)
                        ...
                        @endphp
                            <tr class="hover:bg-[#ecedf7]">
                                ...
                                <td class="px-4 py-3 text-slate-700 text-sm">{{ $detail->gender ?? '-' }}</td>
                                <td class="px-4 py-3 text-slate-700 text-sm">{{ $detail->nis ?? '-' }}</td>
                                <!-- <td class="px-4 py-3 text-slate-700 text-sm">{{ $detail->birth_place ?? '-' }}</td> -->
                                <!-- <td class="px-4 py-3 text-slate-700 text-sm">
                                    {{ ($detail && $detail->birth_date) ? \Carbon\Carbon::parse($detail->birth_date)->format('d-m-Y') : '-' }}
                                </td> -->
                                <!-- <td class="px-4 py-3 text-slate-700 text-sm">{{ $detail->address ?? '-' }}</td> -->
                                <td class="px-4 py-3 text-slate-700 text-sm">
                                ...
                                <td class="px-4 py-3 text-slate-700 text-sm flex flex-row items-center gap-1">
        ...
        <div class="mt-4 flex items-center justify-between text-slate-500 text-sm">
            <div>Showing <span class="text-slate-700">{{ $users->firstItem() }}</span> to <span class="text-slate-700">{{ $users->lastItem() }}</span> of <span class="text-slate-700">{{ $users->total() }}</span> entries</div>
        ...
        data.data.forEach((user) => {
            ...
            const row = `
                <tr class="hover:bg-[#ecedf7]">
                    <td class="px-4 py-3 text-slate-700 text-sm">${user.name}</td>
                    <td class="px-4 py-3 text-slate-700 text-sm">${user.email || '-'}</td>
                    <td class="px-4 py-3 text-slate-700 text-sm">${(detail && detail.gender) ? detail.gender : '-'}</td>
                    <td class="px-4 py-3 text-slate-700 text-sm">${(detail && detail.nis) ? detail.nis : '-'}</td>
