    ...
    <div class="content-section">
        <div class="backdrop-blur-sm rounded-xl px-6 mb-6 flex flex-row items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h3 class="text-2xl font-semibold text-black">Pelanggaran Siswa</h3>
                    <p class="text-slate-700">Daftar pelanggaran siswa — tanggal, jam, status, dan lokasi.</p>
                </div>
            </div>
            <div class="flex items-center">
                <a href="{{ route('dashboard.pelanggaran.exportExcel') }}" class="bg-red-600 hover:bg-red-500 text-white px-3 py-2 rounded text-sm"><i class="fa fa-file-download"></i> Ekspor Excel</a>
            </div>
        </div>


        <div class="backdrop-blur-sm rounded-xl p-4 border border-slate-300">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-4">
                ...
                <div class="flex items-center gap-3 w-full md:w-auto">
                    <input type="search" id="pelanggaran-search-input" placeholder="Cari nama siswa / pelajaran" class="flex-1 md:flex-none bg-white text-slate-200 border border-slate-700 rounded px-3 py-2 text-sm" />
            ...
            <div class="overflow-x-auto -mx-4">
                <table class="min-w-full table-auto border-collapse border border-gray-300">
                    <thead>
                        <tr class="text-left text-slate-700 text-sm uppercase tracking-wider bg-[#efedf7]">
                    ...
                    <tbody class="divide-y divide-gray-300" id="pelanggaran-table-body">
                        @if(isset($violations) && $violations->count())
                            @foreach($violations as $idx => $v)
                                <tr class="hover:bg-[#ecedf7] bg-white border border-gray-300">
                                    <td class="px-4 py-3 text-slate-700 text-sm align-top">{{ $loop->iteration }}</td>
                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div class="view-student text-slate-700 font-medium text-sm">{{ $v->student->name ?? '-' }}</div>
                                                <div class="view-class text-slate-700 font-medium text-sm hidden">{{ $v->student->classes->first()->name ?? '-' }}</div>
                                    ...
                                    <td class="px-4 py-3 text-slate-700 text-sm">
                                        <div class="text-sm text-slate-700">{{ $v->rule->name ?? '-' }}</div>
                                    </td>
                                    <td class="px-4 py-3 text-slate-700 text-sm">
                        ...
                        @else
                            <tr>
                                <td colspan="7" class="text-center text-slate-700 py-6">Tidak ada data pelanggaran.</td>
                            </tr>
                        @endif
                    </tbody>
                </table>
            ...
            <div class="mt-4 flex items-center justify-between text-slate-400 text-sm">
                <div>Showing <span class="text-slate-600">1</span> to <span class="text-slate-600">10</span> entries</div>
