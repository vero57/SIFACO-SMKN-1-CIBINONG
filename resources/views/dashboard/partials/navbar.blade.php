<header class="bg-[#F9F9FF]/85 backdrop-blur-sm border-b border-slate-300 px-2 sm:px-4 md:px-6 py-2 md:py-4 shrink-0">
    <div class="flex items-center justify-between gap-2 md:gap-4">
        <div class="flex items-center gap-2 md:gap-3 min-w-0">
            ...
            <h2 id="pageTitle" class="text-base md:text-2xl font-semibold text-gray-700 truncate">Dashboard</h2>
        </div>
        <div class="flex items-center gap-2 md:gap-4 min-w-0">
            <div class="text-gray-700 flex items-center gap-2 md:gap-3 flex-shrink-0">
                <i class="fas fa-bell text-sm md:text-base"></i>
                @if(auth()->check())
                    <span class="text-xs md:text-sm hidden sm:inline">
                        Halo, <span class="text-gray-500 font-semibold">{{ auth()->user()->name }}</span>!
                    </span>
